...
#Define the model and add in the Lora configuration
model = AutoModelForCausalLM.from_pretrained(hp.model_name, **hp.model_args)

#Define the tokenizer, add in the special tokens, and get the token id of the output token
...
#Add in the lora configuration
model = get_peft_model(model, hp.lora_config)   #NOTE: Make sure to put this AFTER adding in special tokens if you are setting trainable_token_indices in your LoraConfig()
...
train_losses, val_losses, smallest_val_loss = train(
    ...
    hp.initial_val,        #Whether to do a validation loss before starting training
